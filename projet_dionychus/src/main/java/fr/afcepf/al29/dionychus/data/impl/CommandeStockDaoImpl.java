...
public class CommandeStockDaoImpl implements CommandeStockDaoItf {
	...
	public List<CommandeStock> getAll() {
		String SQL = "SELECT cs.id_commande, cs.date_validation, cs.date_reception, cs.numero_reference, cs.id_acteur, cs.id_statut_commande, sc.libelle FROM commande cs INNER JOIN statut_commande sc WHERE cs.id_statut_commande = sc.id_statut_commande";
	...
	public List<CommandeStock> getCommandesByIdUtilisateur(Integer paramIdUtilisateur) {
		String SQL = "SELECT cs.id_commande, cs.date_validation, cs.date_reception, cs.numero_reference, cs.id_acteur, cs.id_statut_commande, sc.libelle FROM commande cs INNER JOIN statut_commande sc WHERE cs.id_statut_commande = sc.id_statut_commande AND cs.id_acteur = ?";
	...
	public List<CommandeStock> getCommandesByIdStatutCommande(Integer paramIdStatutCommande) {
		String SQL = "SELECT cs.id_commande, cs.date_validation, cs.date_reception, cs.numero_reference, cs.id_acteur, cs.id_statut_commande, sc.libelle FROM commande cs INNER JOIN statut_commande sc WHERE cs.id_statut_commande = sc.id_statut_commande AND cs.id_statut_commande = ?";
	...
	public void addCommandeStock(CommandeStock paramCommandeStock) {
		String SQL = "INSERT INTO `bdd_dionychus`.`commande` (`date_reception`, `date_validation`, `numero_reference`, `id_statut_commande`, `id_acteur`) VALUES (?,?,?,?,?)";
	...
	public void updateCommandeStock(CommandeStock paramCommandeStock) {
		String SQL ="UPDATE `bdd_dionychus`.`commande` SET `date_reception`=?, `date_validation`=?, `numero_reference`=?, `id_statut_commande`=?, `id_acteur`=? WHERE `id_commande`=?";
